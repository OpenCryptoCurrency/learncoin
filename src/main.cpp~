...
#if defined(NDEBUG)
# error "Learncoin cannot be compiled without assertions."
#endif
...
//TODO : Copy here Genesis Block Hash ID
uint256 hashGenesisBlock("0x017361b26edaba47fdabc9c2e0606fc79f2d891f0fba58c65dd71d9721dc454b");
...
{
    // Learncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Learncoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Learncoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Learncoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Learncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Learncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x017361b26edaba47fdabc9c2e0606fc79f2d891f0fba58c65dd71d9721dc454b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Learncoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1495525782;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 4902507;
        ...
        assert(block.hashMerkleRoot == uint256("0x73c72fee4580022d9cef6426400ec8e3f83fdcc69f7453d8bd47e3b227689329"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Learncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LearncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LearncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LearncoinMiner(CWallet *pwallet)
{
    printf("LearncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("learncoin-miner");
    ...
    try { loop {
        ...
        printf("Running LearncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LearncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LearncoinMiner, pwallet));
}
